-- Seed Data: Cloud-safe project + membership setup
-- Run this AFTER resetting the database schema.
--
-- IMPORTANT:
-- 1) Create a real user in Supabase Auth first (Dashboard -> Authentication -> Users).
-- 2) Set target_email below to that user's email.
-- 3) Do not use shared test credentials in cloud environments.
...
BEGIN
    IF target_email = 'CHANGE_ME_EMAIL@example.com' THEN
        RAISE EXCEPTION 'Set target_email in supabase/seed_data.sql before running this script';
    END IF;
