-- CityScope: optional WGS84 [lng,lat] route geometry bundle on submission_batches;
-- extend submit_unified_submission_write to persist it on every call.

ALTER TABLE public.submission_batches
    ADD COLUMN IF NOT EXISTS colab_route_geometry_bundle jsonb;

COMMENT ON COLUMN public.submission_batches.colab_route_geometry_bundle IS
    'Optional JSON bundle of route geometry for CityScope export; coordinates are WGS84 [lng, lat].';

DROP FUNCTION IF EXISTS public.submit_unified_submission_write(text, uuid, text, uuid, uuid, jsonb, text);
...
CREATE OR REPLACE FUNCTION public.submit_unified_submission_write(
    ...
    p_colab_route_geometry_bundle jsonb DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF p_colab_route_geometry_bundle IS NOT NULL THEN
        UPDATE public.submission_batches sb
        SET colab_route_geometry_bundle = p_colab_route_geometry_bundle
        WHERE sb.submission_id = p_submission_id;
    END IF;
END;
$$;

REVOKE ALL ON FUNCTION public.submit_unified_submission_write(text, uuid, text, uuid, uuid, jsonb, text, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_unified_submission_write(text, uuid, text, uuid, uuid, jsonb, text, jsonb) TO authenticated;

COMMENT ON FUNCTION public.submit_unified_submission_write(text, uuid, text, uuid, uuid, jsonb, text, jsonb) IS
