-- Palette v2: replace fixed 16 display_color values (index-aligned old[i] -> new[i]) and refresh CHECK + RPC palette.
...
SET display_color = CASE upper(btrim(display_color))
    WHEN '#C1121F' THEN '#E11D48'
    WHEN '#F48C06' THEN '#F97316'
    WHEN '#FFBA08' THEN '#EAB308'
    WHEN '#136F63' THEN '#65A30D'
    WHEN '#3A86FF' THEN '#16A34A'
    WHEN '#8338EC' THEN '#059669'
    WHEN '#FF006E' THEN '#0D9488'
    WHEN '#38B000' THEN '#0891B2'
    WHEN '#0077B6' THEN '#0284C7'
    WHEN '#FB5607' THEN '#2563EB'
    WHEN '#3EC300' THEN '#4338CA'
    WHEN '#7209B7' THEN '#6D28D9'
    WHEN '#00B4D8' THEN '#A21CAF'
    WHEN '#E85D04' THEN '#C026D3'
    WHEN '#9D4EDD' THEN '#DB2777'
    WHEN '#2D6A4F' THEN '#F43F5E'
    ELSE display_color
END
WHERE upper(btrim(display_color)) IN (
    '#C1121F',
    '#F48C06',
    '#FFBA08',
    '#136F63',
